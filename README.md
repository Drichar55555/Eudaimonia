...
- Soul lamps live in `scenes/soul_lamp.tscn` and `scripts/soul_lamp.gd`. They are lamp-shaped checkpoint areas with no solid collision; entering their range saves the game with the same cooldown behavior as the birth save point. Each soul lamp has a mask attribute, and restores no-mask health plus the matching unlocked mask form to full health. The first soul lamp is Euda-aligned by default.
