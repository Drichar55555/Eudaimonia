...
@export var button_color := Color(0.08, 0.1, 0.13, 0.92)
@export var button_hover_color := Color(0.16, 0.18, 0.22, 0.96)
@export var rollback_card_color := Color(0.045, 0.055, 0.07, 0.94)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if _is_key(key_event, KEY_ESCAPE):
		...
		get_viewport().set_input_as_handled()
		return
	if visible and _handle_navigation_key(key_event):
		get_viewport().set_input_as_handled()

func _handle_escape_key() -> void:
	if visible and _view_mode == "confirm_rollback":
		_cancel_rollback_confirmation()
		return
	if visible and _view_mode == "rollback":
		_view_mode = "main"
		_main_selection = 1
		queue_redraw()
	else:
...
func _handle_navigation_key(key_event: InputEventKey) -> bool:
	...
	if _is_key(key_event, KEY_RIGHT):
		_move_selection(Vector2i(1, 0))
		return true
	if _is_key(key_event, KEY_SPACE):
...
func _draw_main_view() -> void:
	var panel_size := Vector2(minf(size.x * 0.74, 520.0), 300.0)
	...
	if font == null:
		return
	_draw_centered_text(font, title_text, panel_rect.position.y + 52.0, panel_rect.size.x, panel_rect.position.x, 34, Color(0.98, 0.96, 0.86, 1.0))
	...
	var button_size := Vector2(panel_size.x - 96.0, 44.0)
	var button_x := panel_position.x + 48.0
...
func _draw_rollback_view() -> void:
	_resolve_save_manager()
	var panel_size := Vector2(minf(size.x * 0.9, 980.0), minf(size.y * 0.84, 660.0))
	var panel_position := (size - panel_size) * 0.5
	var panel_rect := Rect2(panel_position, panel_size)
	_draw_panel(panel_rect)
	...
	draw_string(font, panel_position + Vector2(32.0, 46.0), "回档", HORIZONTAL_ALIGNMENT_LEFT, panel_size.x - 64.0, 30, Color(0.98, 0.96, 0.86, 1.0))
	_draw_button(Rect2(Vector2(panel_rect.end.x - 128.0, panel_rect.position.y + 22.0), Vector2(92.0, 34.0)), "返回", "back", font, 16, _rollback_selection == 0)
	draw_line(panel_position + Vector2(28.0, 70.0), panel_position + Vector2(panel_size.x - 28.0, 70.0), Color(accent_color.r, accent_color.g, accent_color.b, 0.55), 2.0, true)

	if entries.is_empty():
		_draw_centered_text(font, "暂无存档", panel_rect.get_center().y, panel_rect.size.x, panel_rect.position.x, 22, Color(0.78, 0.84, 0.92, 1.0))
		return

	var card_gap := 18.0
	var card_width := (panel_size.x - 64.0 - card_gap * 2.0) / 3.0
	var card_height := panel_size.y - 116.0
	var card_y := panel_position.y + 88.0
	for index in entry_count:
		var card_rect := Rect2(Vector2(panel_position.x + 32.0 + index * (card_width + card_gap), card_y), Vector2(card_width, card_height))
...
func _draw_confirm_rollback_view() -> void:
	...
	var panel_size := Vector2(minf(size.x * 0.72, 440.0), 190.0)
	var panel_position := (size - panel_size) * 0.5
	var panel_rect := Rect2(panel_position, panel_size)
	_draw_panel(panel_rect)
